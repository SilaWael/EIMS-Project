...
def extract_date_from_html(soup, text):
    ...
    # 1. Try class header-badge or header-date
...
def parse_html_report(html_bytes):
    """
    Parses uploaded HTML file and extracts highly structured, distinct records
    by dynamically mapping detailed level tables to their corresponding summary work items.
    """
    ...
    # --------------------------------------------------------------------------
    # FORMAT 1: Single Section Road Subgrade (e.g. 1+000 TO 1+160 LHS 2nd Layer)
    # --------------------------------------------------------------------------
    ...
    # --------------------------------------------------------------------------
    # FORMAT 2: Irrigation Network (e.g. CamScanner 15-05-2026 07.40)
    # --------------------------------------------------------------------------
    ...
    # --------------------------------------------------------------------------
    # FORMAT 3: Multi-Section Daily Audit
    # --------------------------------------------------------------------------
    work_items = []
    summary_rows = tables[0].find_all('tr')
    
    for r in summary_rows:
        if r.find('th') or r.parent.name == 'thead' or r.parent.name == 'tfoot':
            continue
        ...
        if len(tds) >= 4:
            ...
            if "grand total" in desc.lower() or "total" in desc.lower():
                continue
            ...
            if q_match:
                ...
                if q_match_any:
                    try:
                        ...
                        if any(u in q_text.lower() for u in ["duct", "crossing", "structure", "manhole", "joint"]):
                            q_unit = "Unit"
                        else:
                            q_unit = "m"
    ...
    for table_idx, tbl in enumerate(tables[1:], start=1):
        ...
        for item in work_items:
            ...
            for w in desc_words:
                if w in preceding_text.lower():
                    score += 10 # very strong match if found in heading
                elif w in search_context:
                    score += 5
            for w in loc_words:
                if w in preceding_text.lower():
                    score += 4
...
def export_to_excel(df_filtered):
    ...
    # Explicit column selection to avoid any schema ordering discrepancy
    cols_to_export = [
        "id", "report_date", "category", "sub_category", "location", "stationing", 
        "activity_detail", "quantity", "unit", "status", "remarks", "pdf_filename"
    ]
    # Ensure columns exist in the dataframe before exporting
        ...
        if uploaded_html is not None:
            if st.button("🔬 Start Analysis & Data Extraction"):
                with st.spinner("Parsing HTML table structure and analyzing level deviations..."):
                    ...
                    try:
                        extracted = parse_html_report(html_bytes)
                        if extracted:
                            for item in extracted:
                                item["report_date"] = format_date_to_display(item.get("report_date"))
                            st.session_state['parsed_records'] = extracted
    ...
    if st.session_state['parsed_records']:
        ...
        if st.button("💾 Save & Commit All Extracted Records to Master Registry Now"):
            ...
            for rec in records_to_save:
                ...
                # If file_bytes is None, save_record will automatically try to find pdf_name in Processed_Audits!
        ...
        if raw_text_input:
            if st.button("🔬 Start Text Analysis & Parsing", key="btn_parse_text"):
                ...
                try:
                    # 1. Date extraction
                    date_str = datetime.now().strftime("%Y-%m-%d")
                    m_date = re.search(r'(\d{1,2})[-/](\d{1,2})[-/](\d{4})', raw_text_input)
                    if m_date:
                        day, month, year = m_date.groups()
                        date_str = f"{year}-{int(month):02d}-{int(day):02d}"
                    else:
                        m_date_iso = re.search(r'(\d{4})[-/](\d{1,2})[-/](\d{1,2})', raw_text_input)
                        if m_date_iso:
                            year, month, day = m_date_iso.groups()
                            date_str = f"{year}-{int(month):02d}-{int(day):02d}"
                    ...
                    if m_loc:
                        ...
                        first_line = raw_text_input.split("\n")[0].strip()
                        if len(first_line) > 5:
                            location = first_line[:50]
                    ...
                    for line in lines:
                        ...
                        if len(parts) >= 3:
                            try:
                                ...
                                if len(floats) >= 2:
                                    detailed_levels.append({
                                        "point_location": parts[0],
                                        "required_level": floats[0],
                                        "as_built_level": floats[1],
                                        "difference": floats[2] if len(floats) >= 3 else round(floats[1] - floats[0], 3),
                                        "status": "Pass" if abs(floats[1] - floats[0]) <= 0.05 else "Rejected"
                                    })
                            except Exception:
                                pass
                                
                    extracted_recs.append({
                        "report_date": format_date_to_display(date_str),
                        "category": category,
                        ...
                        "detailed_levels": detailed_levels,
                        "stationing": stationing,
                        "activity_detail": raw_text_input[:500] + "..."
                    })
        ...
        if st.session_state['parsed_records']:
            ...
            for idx, r in enumerate(st.session_state['parsed_records']):
                with st.expander(f"⚙️ Extracted Record #{idx+1}: {r['category']}", expanded=True):
                    ...
                    if detailed_list:
                        levels_df = pd.DataFrame(detailed_list)
                        edited_df = st.data_editor(levels_df, use_container_width=True, key=f"editor_txt_{idx}", num_rows="dynamic")
                        edited_levels = edited_df.to_dict(orient='records')
            ...
            if st.button("💾 Save & Commit Spatial Text Record to Registry", key="btn_save_text_records"):
                ...
                for rec in records_to_save:
                    success = save_record(
                        ...
                        status=rec["status"],
                        remarks=rec["remarks"],
                        pdf_file=st.session_state['pdf_file_bytes'],
                        detailed_levels_list=rec["detailed_levels"],
                        original_filename=st.session_state['pdf_file_name'],
                        stationing=rec.get("stationing"),
                        activity_detail=rec.get("activity_detail")
                    )
                    if success:
        ...
        if ai_json_input:
            try:
                clean_json = ai_json_input.strip()
                ...
                if st.button("💾 Save & Commit JSON Record to Registry", key="btn_save_json_record"):
                    if not pdf_uploader:
                        ...
                        success = save_record(
                            ...
                            quantity=float(parsed_data.get("quantity", 0)),
                            unit=parsed_data.get("unit"),
                            status=parsed_data.get("status"),
                            remarks=parsed_data.get("remarks"),
                            pdf_file=pdf_uploader,
                            detailed_levels_list=detailed_list,
                            original_filename=pdf_uploader.name,
                            stationing=parsed_data.get("stationing"),
